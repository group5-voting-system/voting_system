...
app.get('/api/candidates', async (req, res) => {
  try {
    ...
    const partyCandidates = await knex('PARTY_CANDIDATE')
      .select('PARTY_CANDIDATE.*', 'USERS.NAME', 'PARTY_LISTS.LIST_NAME as PARTY_NAME')
...
app.post('/api/local-lists', async (req, res) => {
  try {
    ...
    const [newCandidate] = await knex('LOCAL_CANDIDATE').insert({
      NATIONAL_ID,
      VOTES,
      SEAT_TYPE,
      IS_APROVED: false
    ...
    await knex('USERS').insert({ NATIONAL_ID, NAME }).onConflict('NATIONAL_ID').merge();
...
app.post('/api/party-candidates', async (req, res) => {
  try {
    ...
    let partyList = await knex('PARTY_LISTS').where('LIST_NAME', PARTY_NAME).first();
    if (!partyList) {
      [partyList] = await knex('PARTY_LISTS').insert({ LIST_NAME: PARTY_NAME }).returning('*');
    }

    const [newCandidate] = await knex('PARTY_CANDIDATE').insert({
      NATIONAL_ID,
...
app.post('/api/submit-advertisement', async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Advertisement submitted successfully', id });
  } catch (error) {
